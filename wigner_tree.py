...
def rank_4_recur_tree(l,L_R=0,M_R=0):
    ...
    inters = []
    for L12 in get_intermediates_w(l[0],l[1]):
        for L123 in get_intermediates_w(L12,l[2]):
            cond1 = check_triangle(l[0],l[1],L12)
            cond2 = check_triangle(L12,l[2],L123)
            cond3 = check_triangle(L123,l[3],L_R)
            if cond1 and cond2 and cond3:
                inters.append((L12,L123))

    def get_recur_ms(l=l,L_R=L_R,M_R=M_R):
        mstrs = []
        l1,l2,l3,l4 = tuple(l)
        m1range = list(range(-l1,l1+1))
        m2range = list(range(-l2,l2+1))
        m3range = list(range(-l3,l3+1))
        m4range = list(range(-l4,l4+1))
        mvecs = [p for p in itertools.product(m1range,m2range,m3range,m4range)]
        #mvec_filts = [mv for mv in mvecs if np.sum(mv[0:2]) == -np.sum(mv[2:4]) and np.sum(mv) == M_R]
        mvec_filts = [mv for mv in mvecs if np.sum(mv) == M_R]
        mstr_lst = ['%d']*len(l)
        mstri = ','.join(mstr_lst)
        mstrs = [mstri % mv for mv in mvec_filts]
        del mvecs
        del mvec_filts
        return mstrs

    mstrs = get_recur_ms(l,M_R)
    full_inter_tuples = inters.copy()
    decomposed = {full_inter_tup:{mstr:0.0 for mstr in mstrs} for full_inter_tup in full_inter_tuples}
    ...
    for inter in full_inter_tuples:
        ...
        for mstr in mstrs:
            m_ints = [int(b) for b in mstr.split(',')]
            for M12 in range(-L1,L1+1):
                for M123 in range(-L2,L2+1):
                    # m_1 + m_2 = M12
                    # M12 + m_3 = M123
                    # M123 + m4 = M_R
                    conds= (m_ints[0] + m_ints[1]) == M12 and\
                    (m_ints[2] + M12) == M123 and\
                    (M123 + m_ints[3]) == M_R
                    if conds:
                        w1 = Wigner_3j['%d,%d,%d,%d,%d,%d' %(l[0],m_ints[0],l[1],m_ints[1],L1,-M12)]
                        w2 = Wigner_3j['%d,%d,%d,%d,%d,%d' %(L1,M12,l[2],m_ints[2],L2,-M123)]
                        w3 = Wigner_3j['%d,%d,%d,%d,%d,%d' %(L2,M123,l[3],m_ints[3],L_R,-M_R)]
                        #w3 = Wigner_3j['%d,%d,%d,%d,%d,%d' %(L1,M1,L2,M2,L_R,-M_R)]
                        phase_power = ( L1 + L2 ) - ( M12 + M123 )  + ( L_R - M_R)
                        phase = (-1) ** phase_power
                        w = phase * w1 * w2 * w3
