...
nradbase = 16                                           # For now, this is just the n_max for rank = 1
lmax_dict = {1:1,2:2,3:2,4:2,5:2,6:2}                   # maximum l quantum number in basis functions per rank: {rank:lmax(rank)} (note lmax of rank 1 must be 0 for symmetry reduction)
...
ranked_nus = [generate_nl(rank,nradmax_dict[rank],\
...
import json
with open('lexico_ag.json','w') as writejson:
