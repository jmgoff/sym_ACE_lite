...
def get_spher_pol(r_vecs,r_arr):
	...
	polars = [np.arccos(cart[2]/np.linalg.norm(cart)) for cart in carts]
...
class A_basis:
	...
	#def gen_chem_combs(self):
		# all chemical combinations ij (with repeats)
		#combstrs = ind_vec(self.rb.species)
	...
	def A(self,n,l,m,**kwargs):
		...
		if self.rb.chemflag:
			muj = kwargs['muj']
			#mu delta function - could be replaced with another discrete basis
